...
const SetAmountPage: React.FC = () => {
    ...
    const NumberButton: React.FC<{ number: string; onClick: () => void }> = ({ number, onClick }) => (
        <button
            onClick={onClick}
            className="w-16 h-16 sm:w-18 sm:h-18 rounded-full hover:bg-gray-200 active:bg-gray-300 transition-colors flex items-center justify-center text-xl sm:text-2xl font-medium text-gray-900"
        >
            {number}
        </button>
    );
    ...
    const ActionButton: React.FC<{ children: React.ReactNode; onClick: () => void }> = ({ children, onClick }) => (
        <button
            onClick={onClick}
            className="w-16 h-16 sm:w-18 sm:h-18 rounded-full hover:bg-gray-200 active:bg-gray-300 transition-colors flex items-center justify-center text-gray-600"
        >
            {children}
        </button>
    );
    ...
    return (
        <div className="flex flex-col h-full min-h-screen">
            <div className="h-9/10 md:h-12/12 lg:h-12/12 flex flex-col">
                ...
                {/* Amount Display - Flexible height */}
                <div className="flex-1 flex flex-col items-center justify-center px-4 py-2 min-h-0">
                    <div className="text-center">
                        ...
                        <div className="flex items-center justify-center mb-2 sm:mb-4 px-4">
                            {/* Input invisible para desktop, display para móvil */}
                            {!isMobile ? (
                                <div className="relative inline-flex items-center">
                                    <span className="absolute left-3 text-lg sm:text-xl font-medium text-gray-600 pointer-events-none z-10">
                                    ...
                                    <input
                                        ...
                                        className="text-2xl sm:text-3xl font-light text-gray-900 bg-transparent border border-gray-200 rounded-lg outline-none text-center pl-14 pr-4 py-2 min-w-[200px] max-w-[calc(100vw-8rem)] md:max-w-[400px] focus:border-blue-500 transition-colors"
                                        placeholder="0"
                                        style={{
                                ...
                                <div className="flex items-center">
                                    <span className="text-lg sm:text-xl font-medium text-gray-600 mr-2 flex-shrink-0">
                                        {selectedCurrency?.code || '$'}
                                    </span>
                                    <span className="text-2xl sm:text-3xl font-light text-gray-900 max-w-[calc(100vw-8rem)] md:max-w-[calc(20vw-8rem)]">
                        ...
                        <div className="flex items-center justify-center px-4">
                            ...
                            <span className="text-lg sm:text-xl text-gray-500 truncate max-w-[calc(100vw-8rem)] md:max-w-[calc(20vw-8rem)]">
                                {amountToken || '0.00'}
                            </span>
                ...
                {/* Numeric Keypad - Solo visible en móvil o siempre visible */}
                <div className="flex-shrink-0 pb-2">
                    <div className="grid grid-cols-3 gap-2 sm:gap-4 px-4 w-full justify-items-center">
                        {/* Row 1 */}
