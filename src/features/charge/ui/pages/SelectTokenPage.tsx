...
const SelectTokenPage: React.FC = () => {
  ...
  const passedTokens = location.state?.tokens || [];

  const { tokens, selectedToken, selectToken, handleBuy } = useSelectToken({
    tokens: passedTokens,
  });
    ...
    <div className="flex flex-col h-full p-4">
      <div className="h-9/10 md:h-12/12 lg:h-12/12 flex flex-col">
        ...
        <div className="px-4">
          ...
          <DescriptionApp title="Selecciona tu token para cobrar" />
        </div>
              ...
              <SelectTile
                ...
                title={
                  <div className="flex items-center gap-2">
                    <span
                      className={`font-semibold ${selectedToken === token.id ? "text-white" : "text-[#020F1E]"}`}
                    >
                      {token.symbol} •
                    </span>
                    <span
                      className={`${selectedToken === token.id ? "text-white" : "text-[#495058]"}`}
                    >
                      {token.network}
                    </span>
                    {token.network !== "Solana" && (
                      <span
                        className={`${selectedToken === token.id ? "text-white" : "text-[#495058]"} truncate`}
                      >
                        (Muy pronto)
                      </span>
                    )}
