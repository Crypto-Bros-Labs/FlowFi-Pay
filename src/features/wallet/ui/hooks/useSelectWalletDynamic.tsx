...
export const useSelectWalletDynamic = (props: UseSelectWalletDynamicProps) => {
    ...
    const title = useMemo(() => props.title || location.state?.title, [props.title, location.state?.title]);
    const token = useMemo(() => props.token || location.state?.token, [props.token, location.state?.token]);
    const availableCrypto = useMemo(() => props.availableCrypto !== undefined ? props.availableCrypto : location.state?.availableCrypto, [props.availableCrypto, location.state?.availableCrypto]);
    const showSwitchCoin = useMemo(() => props.showSwitchCoin !== undefined ? props.showSwitchCoin : location.state?.showSwitchCoin, [props.showSwitchCoin, location.state?.showSwitchCoin]);
    const typeTransaction = useMemo(() => (props.typeTransaction || location.state?.typeTransaction || 'buy') as TransactionType, [props.typeTransaction, location.state?.typeTransaction]);
    const onContinue = useMemo(() => props.onContinue || location.state?.onContinue, [props.onContinue, location.state?.onContinue]);
    ...
    const isValidAddress = useCallback((address: string): boolean => {
        ...
        return ethPattern.test(address) || starkPattern.test(address) || solanaPattern.test(address);
    }, []);
    ...
    }, [navigate, title, token, availableCrypto, showSwitchCoin, typeTransaction, onContinue, finalAddress, canContinue]);
    ...
    }, [navigate, location.pathname, title, token, availableCrypto, showSwitchCoin, typeTransaction, onContinue]);
