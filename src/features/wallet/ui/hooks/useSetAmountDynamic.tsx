...
export const useSetAmountDynamic = (
  ...
  const [transferResponse, setTransferResponse] =
    useState<SendCryptoResponseData | null>(null);
  ...
  const [showModalBuyResult, setShowModalBuyResult] = useState(false);
  ...
  const handleInputChange = useCallback(
    (e: React.ChangeEvent<HTMLInputElement>, mode: CurrencyMode) => {
  ...
  const handleKeyDown = useCallback(
    (e: React.KeyboardEvent<HTMLInputElement>) => {
      ...
      if (
        e.key === "." &&
        !(editingMode === "fiat" ? amountFiat : amountToken).includes(".")
      ) {
        return;
      }
    ...
    if (typeTransaction === "transfer") {
      ...
      try {
        ...
        const userUuid =
          (await userRepository.getCurrentUserData())?.userUuid ||
          "default-uuid";
        const bankAccountUuid =
          (await userRepository.getBankAccountUuid()) || "default-bank-account";

        if (bankAccountUuid === "default-bank-account") {
          showDialog({
            title: "Cuenta bancaria requerida",
            subtitle:
              "Por favor, agrega una cuenta bancaria antes de continuar.",
          ...
          switch (response.status) {
            case "UNKNOWN":
              showDialog({
                title: "KYC Requerido",
                subtitle:
                  "No has iniciado tu proceso de KYC o este está incompleto. Por favor, completa tu KYC para continuar con el retiro.",
            ...
            case "REVIEW":
              showDialog({
                ...
                subtitle:
                  "Tu proceso de KYC está en revisión. Por favor, espera a que sea aprobado para continuar con el retiro.",
                nextText: "Aceptar",
                hideBack: true,
              });
              return;
            ...
            case "DECLINED":
              showDialog({
                title: "KYC Rechazado",
                subtitle:
                  "Tu proceso de KYC ha sido rechazado. Por favor, vuelve a intentarlo para continuar con el retiro. Si necesitas ayuda, contacta al soporte.",
                onNext: () => window.open(`${response.kycUrl}`, "_blank"),
