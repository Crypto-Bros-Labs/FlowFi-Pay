...
export const useAddWallet = () => {
  ...
  const validateAddress = useCallback(
    (addr: string, networkId: string): string => {
      if (!addr.trim()) return "";
  ...
  const networkOptions: ComboBoxOption[] = networks.map((network) => ({
    id: network.id,
    component: <TileApp title={network.name} />,
  }));
  ...
  const selectedNetworkComponent = () => {
    ...
    return <TileApp title={selectedNetwork.name} />;
  };
  ...
  const isFormValid = useMemo(() => {
    ...
    const aliasValid =
      walletAlias.trim() !== "" && !validateWalletAlias(walletAlias);
    const addressValid =
      address.trim() !== "" && !validateAddress(address, selectedNetworkId);
    ...
    try {
      ...
      if (!success) {
        ...
        showDialog({
          title: "Error",
          subtitle:
            "No se pudo agregar la wallet. Por favor, intenta nuevamente.",
          nextText: "Aceptar",
          hideBack: true,
        });
      } else {
  ...
  const formattedAddress = address
    ? formatCryptoAddress(address, "medium")
    : address;
